...
接手本项目时请先阅读 [HANDOFF.md](HANDOFF.md)。后续开发路线、样例要求、单入口工具面板方案和测试约定保存在本机的 `AGENTS.md` 中；该文件是 Codex 的本地协作说明，不纳入 GitHub 仓库。
...
- 每次准备推送代码前，都要检查 README 是否仍准确反映当前功能、入口、目录、构建方式和发布状态。
- 代码或用户可见行为发生变化时，README 必须与代码放在同一次提交中更新，保证推送完成后文档同步。
- 即使本次变化不需要修改 README，也应在推送前完成检查并确认无需更新。
- 本机的 `AGENTS.md` 仅用于 Codex 协作，不应提交或上传到 GitHub。
